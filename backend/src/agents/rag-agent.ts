...
export class RAGAgent {
  ...
  private chainPromise: Promise<ReturnType<typeof createRAGChain>> | null =
    null;
  ...
  async invoke(
          ...
          logger.debug(
            {
              ...
              rewrittenQuestion: processedQuestion,
            },
    ...
    const formattedHistory: Array<[string, string]> = previousMessages.map(
      (msg) => [msg.role === "user" ? "human" : "ai", msg.content],
    );
    ...
    logger.info(
      {
        chainInput: {
          question: chainInput.question.substring(0, 100),
          hasHistory: !!chainInput.chat_history,
        },
        language,
      },
      "RAG Agent: Invoking chain",
    );
    ...
    try {
      ...
      logger.info(
        ...
        "RAG Agent: Chain invocation completed with LangChain guardrails",
      );
      return result;
    } catch (error) {
      logger.error(
        { error, question: processedQuestion.substring(0, 100) },
        "RAG Agent: Error invoking chain",
      );
      throw error;
    }
  }
  ...
  async stream(
    ...
    try {
      ...
      logger.debug(
        { totalChunks: chunkCount, finalAnswerLength: fullAnswer.length },
        "RAG Agent Stream: Stream completed",
      );
    } catch (error) {
    ...
    return {
      ...
      metadata: {
        agent: this.name,
        model:
          "modelName" in streamingLLM
            ? (streamingLLM as { modelName: string }).modelName
            : "unknown",
        tokenUsage,
        timings: {
          retrievalMs: searchTimeMs,
