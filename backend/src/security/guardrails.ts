...
export function detectPromptInjection(
  ...
  const patterns = config.blockedPatterns || DEFAULT_CONFIG.blockedPatterns || [];
  ...
  const suspiciousIndicators = [
    ...
    (input.match(/\n{3,}/g) || []).length > 0,
    input.toLowerCase().includes("system:") && input.toLowerCase().includes("respond"),
  ];
...
export function sanitizeInput(
  ...
  if (input.length > config.maxInputLength) {
    warnings.push(`Input truncated from ${input.length} to ${config.maxInputLength} characters`);
    input = input.substring(0, config.maxInputLength);
  }

  const injectionCheck = detectPromptInjection(input, config);
  if (injectionCheck.isInjection) {
    warnings.push(`Potential prompt injection detected: ${injectionCheck.detectedPatterns.join(", ")}`);
    logger.warn(
      {
...
export function validateOutput(
  ...
  if (output.length > config.maxOutputLength) {
    errors.push(`Output exceeds maximum length of ${config.maxOutputLength} characters`);
  ...
  const controlCharCount = (sanitized.match(/[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F]/g) || []).length;
  if (controlCharCount > 10) {
    errors.push(`Output contains excessive control characters: ${controlCharCount}`);
    sanitized = sanitized.replace(/[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F]/g, "");
  }
  ...
  try {
    ...
    for (const [category, flagged] of Object.entries(result.categories)) {
      if (flagged) {
        ...
        const categoryScore = result.category_scores[category as keyof typeof result.category_scores] || 0;
    ...
    logger.error({ error }, "Failed to call OpenAI Moderation API, falling back to safe default");
    return {
      isSafe: false,
...
export function createSecureParserWithRetry(
  llm: BaseChatModel,
  config: GuardrailConfig = DEFAULT_CONFIG,
  _maxRetries: number = 3 // Kept for API compatibility
...
export function createStructuredParserWithGuardrails<T extends z.ZodTypeAny>(
  ...
  return OutputFixingParser.fromLLM(llm, baseParser) as BaseOutputParser<z.infer<T>>;
}
