...
export function initializeDatabase(db: Database.Database): void {
  ...
  const tableInfo = db.prepare("PRAGMA table_info(orders)").all() as Array<{ name: string; notnull: number }>;
  const columnNames = tableInfo.map(col => col.name);
  const customerNameColumn = tableInfo.find(col => col.name === "customer_name");
  ...
  if (customerNameColumn && customerNameColumn.notnull === 0) {
    ...
    db.exec("UPDATE orders SET customer_name = 'Customer Name Required' WHERE customer_name IS NULL");
    // SQLite doesn't support ALTER COLUMN, so we need to recreate the table
    // For now, we'll handle this at insert time, but log a warning
    logger.warn("customer_name column exists but is nullable. New orders will require customer_name.");
  }
  ...
  if (columnNames.includes("company_name")) {
    ...
    logger.debug("company_name column exists but is no longer used in the model");
  }
