...
interface UseAudioRecorderOptions {
  onRecordingComplete?: (audioBlob: Blob, hasSpeech?: boolean, stoppedByMaxTime?: boolean) => void;
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          throw new Error(`Failed to create MediaRecorder: ${err.message || 'Unsupported browser'}`);
        }
      }
      ...
      mediaRecorder.onstop = () => {
        ...
        console.log(`[onstop] Speech detection result: ${hasSpeech ? 'SPEECH DETECTED' : 'NO SPEECH'}, Stopped by max time: ${stoppedByMaxTime}`);
      ...
      try {
        if (mediaRecorder.state === 'inactive') {
          ...
          console.warn(`MediaRecorder state is ${mediaRecorder.state}, expected 'inactive'`);
        }
      } catch (err: any) {
        // Clean up stream
        stream.getTracks().forEach(track => track.stop());
      ...
      if (mediaRecorder.state !== 'recording') {
        stream.getTracks().forEach(track => track.stop());
        throw new Error(`Recording failed to start. State: ${mediaRecorder.state}`);
      }
      ...
      if (autoStop) {
        try {
          ...
          if (!speechDetectorRef.current) {
            try {
              speechDetectorRef.current = new SpeechDetector(audioContext, analyser, microphone);
              console.log('[Speech Detection] FFT-based speech detector initialized in silence detection');
            } catch (error) {
              console.warn('[Speech Detection] Failed to initialize FFT detector in silence detection:', error);
            }
          }
          ...
          const checkSilence = () => {
            ...
            if (speechDetectorRef.current) {
              try {
                ...
                if (isActualSpeech) {
                  ...
                  console.log(`[Speech Detection] ✅ Speech detected - Confidence: ${speechResult.confidence.toFixed(2)}, Centroid: ${speechResult.features.spectralCentroid.toFixed(0)}Hz, MFCC: ${speechResult.features.mfcc.slice(0, 3).map(v => v.toFixed(1)).join(',')}`);
                } else {
                  if (elapsed % 2 < 0.1 && speechResult.confidence > 0.3) {
                    console.log(`[Speech Detection] ⚠️ No speech (confidence: ${speechResult.confidence.toFixed(2)}, threshold: 0.50), Centroid: ${speechResult.features.spectralCentroid.toFixed(0)}Hz`);
                  }
                }
              } catch (error) {
                console.warn('[Speech Detection] Error in FFT detection, falling back to volume-based:', error);
            ...
            if (!speechDetectorRef.current) {
              ...
              const noiseMultiplier = Math.min(backgroundNoiseLevelRef.current || 2, 5);
              const adaptiveThreshold = Math.max(baseThreshold, noiseMultiplier * 1.2);
              
              if (elapsed < 0.5) {
                if (backgroundNoiseLevelRef.current === 0 || average < backgroundNoiseLevelRef.current) {
            ...
            if (elapsed % 1 < 0.1) {
              ...
              const detectionMethod = speechDetectorRef.current ? 'FFT' : 'Volume';
              console.log(`[Silence Detection] Elapsed: ${elapsed.toFixed(1)}s, Volume: ${average.toFixed(1)}, Peak: ${peak.toFixed(1)}, Silence: ${silenceDuration.toFixed(1)}s, SpeechDetected: ${speechDetectedRef.current}, Method: ${detectionMethod}`);
            }
            ...
            if (silenceDuration >= silenceThreshold) {
              console.log(`[Silence Detection] Silence detected for ${silenceDuration.toFixed(1)}s (threshold: ${silenceThreshold.toFixed(2)}s, after ${elapsed.toFixed(1)}s total), stopping recording...`);
              if (silenceTimerRef.current) {
                clearTimeout(silenceTimerRef.current);
              }
              silenceTimerRef.current = window.setTimeout(() => {
                if (mediaRecorderRef.current && isRecordingRef.current) {
                  console.log('[Silence Detection] Auto-stopping recording after silence confirmation');
          ...
          setTimeout(() => {
            if (isRecordingRef.current) {
              console.log(`[Silence Detection] Starting silence detection (threshold: ${silenceThreshold.toFixed(2)}s, will wait for minimum recording time)`);
              animationFrameRef.current = requestAnimationFrame(checkSilence);
            }
          }, 300);
          ...
          if (maxRecordingTime > 0) {
            maxTimeTimerRef.current = window.setTimeout(() => {
              if (mediaRecorderRef.current && isRecordingRef.current) {
                console.log(`[Max Time] Maximum recording time (${maxRecordingTime}s) reached, stopping...`);
      ...
      } else if (autoStop && maxRecordingTime > 0) {
        maxTimeTimerRef.current = window.setTimeout(() => {
          if (mediaRecorderRef.current && isRecordingRef.current) {
            console.log(`[Max Time] Maximum recording time (${maxRecordingTime}s) reached, stopping...`);
            stoppedByMaxTimeRef.current = true; // Mark that we're stopping due to max time
  ...
  const stopRecording = useCallback(() => {
    ...
    if (speechDetectorRef.current) {
      ...
      if (recorder.state === 'recording' || recorder.state === 'paused') {
        console.log('[stopRecording] MediaRecorder state:', recorder.state, '- calling stop()');
        recorder.requestData();
        recorder.stop();
        console.log('[stopRecording] MediaRecorder.stop() called');
      } else {
        console.warn('[stopRecording] MediaRecorder not in recording/paused state:', recorder.state);
      }
