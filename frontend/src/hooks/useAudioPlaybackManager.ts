...
export function useAudioPlaybackManager(options: UseAudioPlaybackManagerOptions = {}) {
  const {
    onPlaybackStart,
    onPlaybackEnd,
    onPlaybackError,
  ...
  const playAudioBlob = useCallback(async (blob: Blob, rate: number) => {
    ...
    if (rate !== 1.0 && 'playbackRate' in audio) {
      ...
      audio.onended = () => {
        ...
        if (currentAudioRef.current?.instance === audio) {
          ...
          console.log('⚠️ Audio ended but was not current audio (already replaced)');
        }
        resolve();
      };
      ...
      audio.play()
        ...
        .catch((playError) => {
          clearTimeout(timeoutId);
          URL.revokeObjectURL(audioUrl);
          const err = playError instanceof Error ? playError : new Error('Failed to play audio');
          onPlaybackError?.(err);
          reject(err);
        });
    });
  ...
  const playBrowserTTS = useCallback(async (text: string, rate: number) => {
