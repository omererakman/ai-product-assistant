      ...
      fetch(`${API_URL}/api/tts/health`)
        .then((res) => {
          if (res.ok) {
            ...
            console.warn('⚠️ Natural TTS health check failed, falling back to browser TTS');
          }
        })
        .catch((error) => {
          setIsSupported(false);
          console.warn('⚠️ Natural TTS backend not available, falling back to browser TTS:', error);
        });
    } else {
      setIsSupported(false);
  ...
  const speak = useCallback(async (text: string) => {
    ...
    if (audioManager.isPlaying) {
      ...
      if (waited >= maxWaitTime) {
        console.warn('⚠️ Audio did not stop within timeout, proceeding anyway');
        audioManager.stop();
      }
    }

    if (enabled && isSupported) {
      console.log(`🎤 Using Natural TTS (OpenAI) with voice: ${voice}, rate: ${rate}`);
      ...
      console.warn('⚠️ Using Browser TTS (robotic). Enable Natural TTS in settings for better quality.');
      await audioManager.play({
        type: 'browser-tts',
        text,
        rate,
